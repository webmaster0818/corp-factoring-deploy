...
echo "📊 フランス SEOデータ取得開始"
echo "認証: $GOOGLE_APPLICATION_CREDENTIALS"
echo ""

node fetch-search-console.js 2>&1 | grep -A 5 "フランス"
node fetch-ga4.js 2>&1 | grep -A 5 "フランス"
node generate-report.js

echo ""
echo "📄 最新レポート:"
cat ../reports/france_report_$(date +%Y-%m-%d).md
